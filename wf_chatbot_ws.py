...
def handle_question(data):
    """Process an incoming question from Pusher.
    Writes question to file for Claude Code session to pick up,
    waits for answer file, sends response back via Pusher."""
    try:
        msg = json.loads(data)
    ...
    log.info(f"[{session_id}] Q: {question[:80]} (mode={mode})")
    start = time.time()

    # Send immediate "thinking" acknowledgment
    try:
        pusher_server.trigger(response_channel, 'thinking', {
            'message': 'Received your question, starting analysis...'
        })
    except Exception:
        pass
    ...
    if cached:
        ...
        log.info(f"[{session_id}] Cache hit ({elapsed_ms}ms)")
        _send_answer(response_channel, session_id, cached, None, ['cache'], elapsed_ms)
        log_conversation(session_id, question, mode, ['cache'], len(cached), elapsed_ms, cached=True)
    ...
    # Find similar prior answer as context (not as final answer)
    prior_q, prior_a = find_similar_context(question, mode)

    # Build conversation history for this session
    history = SESSION_HISTORY.get(session_id, [])
    # Compact history: truncate answers to keep file size manageable
    compact_history = []
    ...
    while time.time() - poll_start < ANSWER_TIMEOUT:
        # Send thinking update every 15s
        ...
        if PENDING_A_FILE.exists():
            try:
                ...
                PENDING_A_FILE.unlink()  # Clean up
...
def main():
    ...
    pysher_client = pysher.Pusher(PUSHER_KEY, cluster=PUSHER_CLUSTER)

    def on_connect(data):
        log.info("Connected to Pusher")
        channel = pysher_client.subscribe('wf-queries')
        channel.bind('question', lambda data: threading.Thread(
            target=handle_question, args=(data,), daemon=True
        ).start())
        log.info("Subscribed to wf-queries channel — listening for questions...")

    pysher_client.connection.bind('pusher:connection_established', on_connect)
